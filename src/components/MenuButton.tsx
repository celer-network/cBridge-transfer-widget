...
export default function MenuButton(): JSX.Element {
  ...
  const { walletAddress, connected, walletConnectionButtonTitle } = useWalletConnectionContext();
  ...
  const menu = (
    <Menu className={classes.dropDownMenu}>
      ...
      {connected ? (
        ...
        <Menu.Item key="addr" className="menuAddress" onClick={showWalletConnectionProviderModal}>
      ...
      {connected && (
        <Menu.Item key="Logout" className="menuAddress" onClick={walletConnectionLogout}>
          Logout
        </Menu.Item>
      )}
    </Menu>
