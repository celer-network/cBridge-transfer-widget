import * as React from "react";

import AntdIcon, { AntdIconProps } from "@ant-design/icons/lib/components/AntdIcon";

const TelegramCircleFilledSvg = {
  icon: {
    tag: "svg",
    attrs: {
      width: "48",
      height: "48",
      viewBox: "0 0 48 48",
      focusable: "false",
      fill: "currentColor",
      fillRule: "evenodd",
      clipRule: "evenodd",
    },
    children: [
      {
        tag: "path",
        attrs: {
          d:
            "M0 24C0 37.2548 10.7452 48 24 48C37.2548 48 48 37.2548 48 24C48 10.7452 37.2548 0 24 0C10.7452 0 0 10.7452 0 24ZM19.6 35L20.0083 28.8823L20.008 28.882L31.1369 18.839C31.6253 18.4055 31.0303 18.1941 30.3819 18.5873L16.6473 27.2523L10.7147 25.4007C9.4335 25.0084 9.4243 24.128 11.0023 23.4951L34.1203 14.5809C35.1762 14.1015 36.1953 14.8345 35.7922 16.4505L31.8552 35.0031C31.5803 36.3215 30.7837 36.6368 29.68 36.0278L23.6827 31.5969L20.8 34.4C20.7909 34.4088 20.7819 34.4176 20.7729 34.4264C20.4505 34.7403 20.1837 35 19.6 35Z",
        },
      },
    ],
  },
  name: "telegram-circle",
  theme: "filled" as const,
};

const TelegramCircleFilled = (props: AntdIconProps, ref: React.MutableRefObject<HTMLSpanElement>) => (
  /* eslint-disable-next-line react/jsx-props-no-spreading */
  <AntdIcon {...props} ref={ref} icon={TelegramCircleFilledSvg} />
);

TelegramCircleFilled.displayName = "TelegramCircleFilled";
export default React.forwardRef<HTMLSpanElement, AntdIconProps>(
  TelegramCircleFilled as React.ForwardRefRenderFunction<HTMLSpanElement, AntdIconProps>,
);
